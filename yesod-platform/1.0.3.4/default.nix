...
, fast-logger, file-embed, hamlet, hashable, hjsmin, hspec
, http-conduit, http-date, http-types, HUnit, hxt
...
, wai-test, warp, xml-conduit, xml-types, xml2html, xss-sanitize
...
mkDerivation {
  ...
  libraryHaskellDepends = [
    ...
    data-default dlist email-validate entropy enumerator failure
    fast-logger file-embed hamlet hashable hjsmin hspec http-conduit
    http-date http-types HUnit hxt hxt-charproperties
    hxt-regex-xmlschema hxt-unicode language-javascript largeword
    lifted-base mime-mail monad-control network-conduit path-pieces pem
    persistent persistent-template pool-conduit primitive pureMD5
    pwstore-fast QuickCheck ranges regex-compat resource-pool resourcet
    safe semigroups SHA shakespeare shakespeare-css shakespeare-i18n
    shakespeare-js shakespeare-text silently simple-sendfile skein
    socks stm system-filepath tagged tagsoup tagstream-conduit tls
