...
, base16, binary, bytestring, cereal, containers, crypton
...
, quickcheck-instances, random, regex, regex-tdfa, rosezipper
...
mkDerivation {
  ...
  libraryHaskellDepends = [
    ...
    cereal containers crypton data-dword Decimal directory exceptions
    extra filemanip filepath filepattern megaparsec memory monad-loops
    mtl operational optics-core optics-extra optics-th optparse-generic
    pretty-hex process QuickCheck random regex-tdfa rosezipper
    scientific smt2-parser spawn split spool stm system-cxx-std-lib
    tasty tasty-expected-failure tasty-hunit template-haskell temporary
    text time transformers tree-view unliftio-core vector witch
    witherable wreq
  ];
